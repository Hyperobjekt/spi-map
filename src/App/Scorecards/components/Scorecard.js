...
const getDataMetricTree = (metric, data) => {
  ...
  const children = metric.children?.map((child) => getDataMetricTree(child, data));
  return {
    ...metric,
    dataValue,
    dataRank,
  ...
  return (
    <Box component="li" className={clsx(className, 'scorecard__row')} {...props}>
      ...
      <Box className="scorecard__perf-wrapper">
        <PerformanceIndicator className="scorecard__perf-circle" performance={dataPerformance} />
      </Box>
    </Box>
  );
};
...
const Scorecard = ({ location, metrics, className, ...props }) => {
  ...
  const [name, parent] = location ? getLocationNameParts(location) : [null, null];
  return (
    <Paper className={clsx('scorecard__root', className)} {...props}>
      <div className="scorecard__header">
        <div style={{ backgroundColor: location?.color }} className="scorecard__location-color" />
        <Typography variant="h5">{name}</Typography>
        <Typography variant="body1" color="textSecondary">
          {parent}
        </Typography>
      </div>
      <ScorecardList items={items} collapseDepths={[]} childComponent={ScorecardItem} />
    </Paper>
  );
};
